...
%.proxy.cpp %.stub.cpp:%.idl %.json $(RPCGEN)
	$(RPCGEN) $<
...
%.proxy.o: %.proxy.cpp $(INCLUDES)
	$(CPP) -c $(CPPFLAGS) $(RPCGENFLAGS) $<

%.stub.o: %.stub.cpp $(INCLUDES)
	$(CPP) -c $(CPPFLAGS) $(RPCGENFLAGS) $<
...
clean:
	 rm -f pingstreamclient pingstreamserver idldeclarationtst idl_to_json simplefunctionclient simplefunctionserver *.o *.json *.pyc $(TARGETS)
