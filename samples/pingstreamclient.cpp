...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    c150debug->printf(C150APPLICATION, "%s: Writing message: \"%s\"", argv[0],
                      argv[msgArg]);
    sock->write(argv[msgArg], strlen(argv[msgArg]) + 1); // +1 includes the null

    // Read the response from the server
    c150debug->printf(C150APPLICATION, "%s: Returned from write", argv[0]);
    ...
    //
    if (readlen == 0) {
      c150debug->printf(C150APPLICATION, "%s: UNEXPECTED 0 LENGTH READ",
                        argv[0]);
      if (sock->eof()) {
        c150debug->printf(C150APPLICATION, "%s: REASON WAS UNEXPECTED EOF",
                          argv[0]);
      }
      if (sock->timedout()) {
        c150debug->printf(C150APPLICATION, "%s: REASON WAS UNEXPECTED TIMEOUT",
                          argv[0]);
      }
...
void checkAndPrintMessage(ssize_t readlen, char *msg, ssize_t bufferlen) {
  ...
  if (msg[readlen - 1] != '\0') {
    throw C150NetworkException(
        "Client received message that was not null terminated");
  };
  ...
  //
  // Note: cleanString wants a C++ string, not a char*, so we make a temporary
  // one here. Not super-fast, but this is just a demo program.
  string s(msg);
  cleanString(s);
  ...
  c150debug->printf(C150APPLICATION,
                    "PRINTING RESPONSE: Response received is \"%s\"\n",
                    s.c_str());
