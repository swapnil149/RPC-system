...
void __func1() {
  ...
  //
  c150debug->printf(C150RPCDEBUG, "simplefunction.stub.cpp: returned from  "
                                  "func1() -- responding to client");
...
void __badFunction(char *functionName) {
  ...
  c150debug->printf(
      C150RPCDEBUG,
      "simplefunction.stub.cpp: received call for nonexistent function %s()",
      functionName);
  RPCSTUBSOCKET->write(doneBuffer, strlen(doneBuffer) + 1);
}
...
void getFunctionNameFromStream(char *buffer, unsigned int bufSize) {
  ...
  if (readlen == 0) {
    c150debug->printf(
        C150RPCDEBUG,
        "simplefunction.stub: read zero length message, checking EOF");
    if (RPCSTUBSOCKET->eof()) {
      c150debug->printf(C150RPCDEBUG,
                        "simplefunction.stub: EOF signaled on input");

    } else {
      throw C150Exception(
          "simplefunction.stub: unexpected zero length read without eof");
    }
  }
  ...
  //
    ...
    throw C150Exception(
        "simplefunction.stub: method name not null terminated or too long");
