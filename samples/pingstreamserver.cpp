    ...
    try {
        ...
        while (1) {
            ...
            while (1) {
                ...
                //
                readlen =
                    sock->read(incomingMessage, sizeof(incomingMessage) - 1);

                //
                // With TCP streams, we should never get a 0 length read
                // except with timeouts (which we're not setting in
                // pingstreamserver) or EOF
                //
                if (readlen == 0) {
                    c150debug->printf(C150APPLICATION,
                                      "Read zero length message, checking EOF");
                    if (sock->eof()) {
                        c150debug->printf(C150APPLICATION,
                                          "EOF signaled on input");
                        break;
                    } else {
                        cerr << "pingstreamserver: unexpected 0 length read "
                                "without EOF. Giving up!"
                             << endl;
                        return 8;
                    }
                }
                ...
                if (incoming == "ping" || incoming == "Ping" ||
                    incoming == "PING")
            ...
            //
            c150debug->printf(C150APPLICATION,
                              "Calling C150StreamSocket::close");
            sock->close();
        }
    }
