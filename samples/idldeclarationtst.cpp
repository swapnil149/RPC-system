...
//        NEEDSWORK:
//
//        This test program and the parsing framework were written in some
//        haste. There will likely be some problems especially with edge cases.
//
...
void processIDLFile(const char fileName[]) {
  ...
  for (iter = parseTree.types.begin(); iter != parseTree.types.end(); ++iter) {
    ...
    typep->to_string_stream(
        formattedType); // format the type info into the buffer
